...
const Companyproduct = () => {
  ...
  const [form2, setform2] = useState({ units: "" })
  ...
  const getAllbenners = async () => {
    const resonse = await addData("companyproduct/getallcompanyproducts")
    var _data = resonse
  ...
  const addbenners = async e => {
    ...
    dataArray.append("units", form.units)
    for (let i = 0; i < Files.length; i++) {
      dataArray.append("image", Files[i])
    }
    try {
      const resonse = await addData(
        "companyproduct/addcompanyproduct",
        dataArray
      )
      var _data = resonse
      console.log(_data)
  ...
  const editbenners = async e => {
    ...
    dataArray.append("units", form1.units)
    ...
    try {
      const resonse = await updateData(
        "companyproduct/editcompanyproduct/" + form1._id,
        dataArray
      )
      var _data = resonse
      console.log(_data)
  ...
  const addmoreunits = async e => {
    ...
    try {
      const resonse = await addData(
        "companyproduct/addmorecompanyproducts",
        bodydata
      )
      var _data = resonse
      console.log(_data)
  ...
  const deletebenners = async data => {
    try {
      const resonse = await deletedData(
        "companyproduct/deletecompanyproduct/" + data._id,
        {}
      )
      var _data = resonse
      console.log(_data)
  ...
  const handleSearch = async e => {
    const resonse = await addData(
      "companyproduct/getallcompanyproducts?searchQuery=" + e.target.value
    )
    var _data = resonse
    setbanner(_data.data.products)
  }
  ...
  const getByfunction = data => {
    sessionStorage.setItem("companyproductid", data._id)
    history.push("/companyproductview")
  }
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumbs */}
          <Breadcrumbs title="Shicar" breadcrumbItem="Company Products" />
